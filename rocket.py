...
class WaterThruster:
    ...
    def step(self, P_gauge: float, d_t: float, acceleration: np.ndarray) -> Tuple[float, float]:
        ...
        F_internal = (
            -water_density
            * self.A_out
            * (self.z_H * du_dt + self.A_out / self.A(self.z_H) * self.u_out**2)
        )
...
class BallisticOld(Phase):
    def __init__(
        self, position, velocity, t0, dry_mass, C_drag, A_cross_sectional_area, timestep=0.001
    ):
        """
        Starting conditions:
...
class BoosterScienceBits(RocketComponent):
    ...
    def __init__(
        ...
        dry_mass,
        volume,  # in liter, total volume
        C_drag,
        A_cross_sectional_area,
        nozzle_radius,  # in meters
        ...
        self.volume_0 = (
            self.volume - water / 1000 - self.nozzle_area * launch_tube_length
        )  # air volume at t0
        ...
        if bottle_shape == "naive":
            ...
            self.thruster = WaterThruster(
                nozzle_profile=nozzle_profile, starting_water_volume=water / 1000
            )
    ...
    def step(self, acceleration, distance_from_origin) -> bool:
        ...
        if in_launch_tube_phase:
            self.f_thrust = (
                self.nozzle_area * pressure
            )  # https://www.et.byu.edu/~wheeler/benchtop/pix/thrust_eqns.pdf Section III
            d_mass = 0
        else:
        ...
        launch_pipe_volume_lost = (
            min(self.launch_tube_length, distance_from_origin) * self.nozzle_area
        )
