...
class Param:
    def __init__(self, lower=None, upper=None, param=None):
        ...
        self.val = cap(
            (self.upper - self.lower) * random.random() + self.lower, self.lower, self.upper
        )
    ...
    def mutate(self, temperature):
        ...
        """
        new_val = cap(
            self.val + random.normalvariate(0.0, temperature * self.var), self.lower, self.upper
        )
        self.val = new_val
...
class GeneticAlgorithm:
    ...
    def run(self):
        ...
        for k in sorted(minion.params):
            v = minion.params[k]
            if isinstance(v, Param):
                print(f"     {k} = {v.val:0.03f}, # optimised")
            else:
                print(f"     {k} = {v:0.03f},")
