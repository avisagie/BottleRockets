...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--frame_index", dest="frame_index", type=int, help="index of the frame to display"
    )
    ...
    if frame_index < 0 or frame_index > len(frames):
        raise ValueError(
            f"Invalid frame index of {frame_index} was selected for video with {len(frames)} frames."
        )
