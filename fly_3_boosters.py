...
from rocket_architectures import plot_basic, sim_rocket_with_boosters, sim_single_bottle
from util import *
...
if __name__ == "__main__":
    ...
    traces = sim_rocket_with_boosters(
        C_drag=0.320,
    ...
    print(
        f"Distance and speed: max height:{np.max(position[:,1]):0.1f}m, distance:{np.max(position[:,0]):0.0f}m, "
