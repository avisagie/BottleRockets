...
dwater = 1000.0 * ureg.kilogram / ureg.meter**3  # [kg/m^3] # water density
...
Rgas = (
    287.058 * ureg.joule / ureg.kilogram / ureg.kelvin
)  # [J/kg/K] ideal gas constant divided by gas molecular weight
...
class bottle_rocket(object):
    ...
    def __init__(
        self, p0, V_water, V_bottle, d_bottle, d_nozzle, m_bottle, Cdrag, dt=0.001 * ureg.second
    ):
        # Model parameters
        ...
        a0 = 0.0 * ureg.meter / ureg.second**2
        v0 = 0.0 * ureg.meter / ureg.second
        s = 0.0 * ureg.meter  # [m], height of rocket above ground
        text = ""
        ...
        self.hmax = 0.0 * ureg.meter
        self.thmax = 0.0 * ureg.second
        self.vmax = 0.0 * ureg.meter / ureg.second
        self.tvmax = 0.0 * ureg.second
        self.amax = 0.0 * ureg.meter / ureg.second**2
        self.tamax = 0.0 * ureg.second
        self.jmax = 0.0 * ureg.meter / ureg.second
        self.tjmax = 0.0 * ureg.second

        # let the rocket fly - max 1000 time steps
        # for i in range(1000):
        while s >= 0.0 * ureg.meter:
            p, V, v, a, vj, ds, dt1, text = self.doTimeStep(
                dt, p, V, self.Vtot, v, a, self.A, self.Q, self.M, dwater, dair, self.Cdrag
            )
            t = t + dt1
            s = s + ds
    ...
    def doTimeStep(self, dt, p0, V0, Vtot, v0, a0, A, Q, M, dwater, dair, Cdrag):
        ...
        if p0 <= pAtm:
            ...
            text = "{0:3.0f} {1:.3f} {2:.3f} {3:.1f} {4:.2f}".format(
                0, 0, (M + m).magnitude, D.magnitude, adrag.magnitude
            )
            ...
            if V0 >= Vtot:
                ...
                if p0 > pAtm:
                    ...
                    if dt0 > dt:
                        vj = (np.sqrt(2.0 * (p0 - pAtm) / dair)).to_base_units()
            ...
            text = "{0:3.0f} {1:.3f} {2:.3f} {3:.1f} {4:.2f}".format(
                T.magnitude, dm.magnitude, (M + m).magnitude, D.magnitude, adrag.magnitude
            )
    ...
    def plot_results(self, fig=0):
        ...
        resname = "rocket-results-{0:d}-{1:d}.pdf".format(
            int(self.p0.magnitude), int(self.V0.magnitude * 1e6)
        )
        plt.savefig(resname)
    ...
    def doPrintStep(self, t, p, V, v, a, ds, vj, dt, text):
        print(
            "{0:5.3f} {1:8.1f} {2:7.2e} {3:7.2f}   {4:7.2f} {5:7.2e} {6:7.3e} {7:.3f} {8:s}".format(
                t.magnitude,
                p.magnitude,
                V.magnitude,
                v.magnitude,
...
if __name__ == "__main__":
    ...
    for v_water in np.arange(0, upper, water_step.magnitude):
        ...
        ax.xaxis.set_units(ureg.seconds)
        ax.plot(
            br_1.timelist, br_1.heights, label=str((br_1.Vwater.to(ureg.liter)).magnitude) + " ℓ"
        )
        ax.set_xlabel("Time [s]")
        ax.set_ylabel("Height [m]")
